    ...
    <div class="flex justify-between items-center px-4 container mx-auto h-full">
      ...
      <div class="relative">
        ...
        >
          ...
          >
            <option @click="changeLanguage('en')">English</option>
            <option @click="changeLanguage('th')">Thai</option>
            <option @click="changeLanguage('la')">Lao</option>
          </select>
        </div>
      </div>
    </div>
...
function changeLanguage(lang) {
  ...
  window.location.reload() // Reload to reinitialize with new language
}
// Toggle the dropdown
