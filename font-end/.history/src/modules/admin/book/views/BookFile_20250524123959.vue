<template>
    ...
    <div class="p-6 bg-white shadow-md rounded-lg">
          ...
          <div class="flex sm:flex-row gap-2 sm:gap-4">
            ...
            <div class="py-2 px-4 whitespace-nowrap flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
              ...
              <div class="flex flex-col sm:flex-row items-stretch sm:items-center ">
                <div bg-green-600 shadow-md text-white px-2 py-1 h-9 focus:outline-none cursor-pointer rounded-l sm:rounded-l>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full bg-white shadow-md rounded-lg text-sm md:text-base">
          ...
          <tbody>
            <tr v-for="file in currentBookFiles" :key="file.id" @click="selectBookFile(file)"
              :class="['border-b cursor-pointer text-center', selectedBookFile?.id === file.id ? 'bg-green-600 text-white' : '']">
  ...
  <!-- Modals -->
  <AddBookFileModal :show="isModalOpen" :books="bookList" :booktypes="bookTypeList" @close="isModalOpen = false"
    @save="saveBookFile" />
