    ...
    <div class="bg-white rounded-lg shadow-md w-full max-w-lg p-6 relative">
      ...
      <form @submit.prevent="handleSave">
        ...
        <div class="mb-3">
          ...
          <select class="form-select w-full border border-gray-300 rounded p-2" v-model="selectedBookType.id" required >
            <option value="">ເລືອກປະເພດປື້ມ</option>
            <option v-for="bookType in props.booktypes" :key="bookType.id" :value="bookType.id">{{ bookType.name}}</option>
          </select>
          </div>
        <!-- Buttons -->
...
function handleSave() {
  if (!formBookFile.value.book_id || !formBookFile.value.file) {
   ...
   if (!selectedBookType.value.id || !selectedBookType.value.name) {
    console.log('Please select a booktype .')
    return
  }

  emit('save', { ...formBookFile.value,...selectedBookType.value})
}
</script>