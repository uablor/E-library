<template>
  ...
  @click="selectedUser && openEditModal()"
  :disabled="!selectedUser || !isEditModalOpen"
  :class="{ 'opacity-50 cursor-not-allowed': !selectedUser || !isEditModalOpen }">ເເກ້ໄຂ</button>
              <button
                class="cursor-pointer bg-red-500 text-white px-8 py-1 rounded hover:bg-red-700 w-full sm:w-auto w-50"
                @click="deleteUser" :disabled="!selectedUser || isEditModalOpen"
                :class="{ 'opacity-50 cursor-not-allowed': !selectedUser || isEditModalOpen }">ລືບ</button>
              <button
...
/*************  ✨ Windsurf Command ⭐  *************/
/**
 * Handle user update event. If the user with the given id exists in the user list, update the user data.
 * @param {User} userData - The user data to be updated.
/*******  e6ec68bf-dfa6-475f-97a3-93d8bf81def1  *******/
