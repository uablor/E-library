        ...
        <table class="min-w-full bg-white shadow-md rounded-lg text-sm md:text-base">
          ...
          <tbody v-if="isSoftDeleteOpen">
            <tr v-for="user in filteredUsers" :key="user.id" @click="selectUser(user)"
              ...
              <td class="py-2 px-4 whitespace-nowrap"><i :class="['fa-solid',user.is_active ? 'fa-check' : 'fa-circle-exclamation',selectedUser && selectedUser.id === user.id ? 'text-white'                    : user.is_active
                      ? 'text-green-600'
                      : 'text-yellow-600'
                ]"></i>
              </td>
