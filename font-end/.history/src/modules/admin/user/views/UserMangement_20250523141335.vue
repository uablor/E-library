<template>
    ...
    <div class="p-6 bg-white shadow-md rounded-lg">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full bg-white shadow-md rounded-lg text-sm md:text-base">
          ...
          <tbody>
            <tr v-for="user in filteredUsers" :key="user.id" @click="selectUser(user,secondArgument)"
  ...
  <!-- Modal addUser -->
  <AddUserModal :show="isModalOpen" :boolenstatus="inbtnEdit_Delete" :user="selectedUser" @close="isModalOpen = false"
    @save="saveUser" @changeBoolenstatus="inbtnEdit_Delete = $event" />
...
const selectUser = (user: User,newValue: User[]) => {
  selectedUser.value = user
...
//   btnEdit_Delete.value = newValue.length === 1 ? false : true;
// });
